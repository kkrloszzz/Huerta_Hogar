...
const UserForm = () =>{
    const [form, setForm] = useState({nombre:"",clave:"",correo:"", rut:"",Telefono:""});
    ...
    const handleSubmit = async e => {
        e.preventDefaoult();
        const {run,nombre,correo,clave,Telefono} = form;
        if(!validarRUT(run)) return setMgs("Rut Incorrecto...");
        if(!nombre) return setMgs("Nombre esta vacio...");
        if(!validarEmail(correo)) return setMgs("Correo incorrcto...");
        if(!clave) return setMgs("Clave vacia");
